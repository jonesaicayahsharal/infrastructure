...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    axios.get(`${API}/products/${id}`).then(r => setProduct(r.data)).catch(e => console.error("Error fetching product:", e)).finally(() => setLoading(false));
  }, [id]);
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-12">
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="relative">
            <div className="aspect-square overflow-hidden bg-royal-900 border border-royal-700"><img src={product.image_url} alt={product.name} className="w-full h-full object-cover" /></div>
            {discount > 0 && <div className="absolute top-4 left-4 bg-gold-500 text-royal-950 px-4 py-2 text-lg font-bold">SAVE {discount}%</div>}
          </motion.div>
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
            <span className="text-gold-400 text-sm font-bold uppercase tracking-widest">{product.category}</span>
            <h1 className="font-heading font-bold text-3xl md:text-4xl text-white mt-2 mb-6">{product.name}</h1>
            <div className="flex items-baseline gap-4 mb-6"><span className="text-gold-400 font-bold text-4xl">{formatPrice(product.sale_price)}</span>{product.regular_price > product.sale_price && <span className="text-royal-500 line-through text-xl">{formatPrice(product.regular_price)}</span>}</div>
            <p className="text-royal-300 text-lg mb-8 leading-relaxed">{product.description}</p>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button onClick={addToCart} className="flex-1 btn-gold py-4 flex items-center justify-center gap-3 text-lg rounded-none disabled:opacity-50 disabled:cursor-not-allowed" disabled={unavailable} data-testid="add-to-cart-btn"><ShoppingCart size={24}/>{unavailable ? "Sold Out" : product.backorder ? "Pre-Order" : "Add to Cart"}</button>
              <Link to="/quote" className="flex-1 btn-outline py-4 text-center text-lg rounded-none">Request Quote</Link>
            </div>
            <div className="mt-8 p-6 glass-card rounded-none border border-royal-700/50"><h4 className="font-heading font-semibold text-white mb-4">Why Buy From Jonesaica?</h4><ul className="space-y-2"><li className="flex items-center gap-2 text-royal-400"><CheckCircle className="text-gold-500" size={16}/>Competitive prices - we beat local competition</li><li className="flex items-center gap-2 text-royal-400"><CheckCircle className="text-gold-500" size={16}/>Professional installation available</li><li className="flex items-center gap-2 text-royal-400"><CheckCircle className="text-gold-500" size={16}/>Island-wide delivery & service</li></ul></div>
          </motion.div>
        </div>
      </div>
    </div>
