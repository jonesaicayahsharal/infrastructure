...
export const ServiceCard = ({ service, index = 0, size = "normal" }) => {
  ...
  return (
    <motion.div
      ...
      className={`service-card glass-card rounded-lg overflow-hidden group ${
    ...
    >
      ...
      <div className={`relative overflow-hidden ${isLarge ? "aspect-[16/9]" : "aspect-video"}`}>
        ...
        {/* Icon */}
        <div className="absolute top-4 left-4 p-3 glass rounded-lg">
      ...
      {/* Content */}
      <div className="p-6">
        <h3 className={`font-heading font-semibold text-white mb-3 ${isLarge ? "text-2xl" : "text-xl"}`}>
          {service.title}
        </h3>
        <p className={`text-slate-400 mb-4 ${isLarge ? "text-base" : "text-sm"} line-clamp-3`}>
          {service.description}
        </p>
